...
void get_bedge_q_omp4_kernel(
  ...
  #pragma omp target teams num_teams(num_teams) thread_limit(nthread) map(to:data0[0:dat0size],data1[0:dat1size]) \
    ...
    map(to:col_reord[0:set_size1],map2[0:map2size],data2[0:dat2size],data3[0:dat3size],data4[0:dat4size],data5[0:dat5size],data6[0:dat6size])
  ...
  for ( int e=start; e<end; e++ ){
    ...
    const double *fscale = &data4[15 * map2idx];
    const double *q = &data5[60 * map2idx];
    double *flux = &data6[60 * map2idx];
    ...
    if(*bedge_type == 0) {
      ...
      for(int i = 0; i < 5; i++) {
        exteriorQ[i * 4]     = bc_r_ompkernel;
        exteriorQ[i * 4 + 1] = bc_r_ompkernel * bc_u_ompkernel;
        exteriorQ[i * 4 + 2] =  bc_r_ompkernel * bc_v_ompkernel;
        exteriorQ[i * 4 + 3] = bc_e_ompkernel;
      }
