...
void save_solution(string filename, int numPts, int numCells, double *q, int *cellMap, int gam) {
  ...
  for(int i = 0; i < numCells; i++) {
    ...
    rho[node0]  = max(q[qCellInd + 4 * qNode0Ind], rho[node0]);
    velX[node0] = max(q[qCellInd + 4 * qNode0Ind + 1] / rho[node0], velX[node0]);
    velY[node0] = max(q[qCellInd + 4 * qNode0Ind + 2] / rho[node0], velY[node0]);
    p[node0]    = max((gam - 1) * (q[qCellInd + 4 * qNode0Ind + 3] - 0.5 * (q[qCellInd + 4 * qNode0Ind + 1] * velX[node0] + q[qCellInd + 4 * qNode0Ind + 2] * velY[node0])), p[node0]);

    rho[node1]  = max(q[qCellInd + 4 * qNode1Ind], rho[node1]);
    velX[node1] = max(q[qCellInd + 4 * qNode1Ind + 1] / rho[node1], velX[node1]);
    velY[node1] = max(q[qCellInd + 4 * qNode1Ind + 2] / rho[node1], velY[node1]);
    p[node1]    = max((gam - 1) * (q[qCellInd + 4 * qNode1Ind + 3] - 0.5 * (q[qCellInd + 4 * qNode1Ind + 1] * velX[node1] + q[qCellInd + 4 * qNode1Ind + 2] * velY[node1])), p[node1]);

    rho[node2]  = max(q[qCellInd + 4 * qNode2Ind], rho[node2]);
    velX[node2] = max(q[qCellInd + 4 * qNode2Ind + 1] / rho[node2], velX[node2]);
    velY[node2] = max(q[qCellInd + 4 * qNode2Ind + 2] / rho[node2], velY[node2]);
    p[node2]    = max((gam - 1) * (q[qCellInd + 4 * qNode2Ind + 3] - 0.5 * (q[qCellInd + 4 * qNode2Ind + 1] * velX[node2] + q[qCellInd + 4 * qNode2Ind + 2] * velY[node2])), p[node2]);
  }
