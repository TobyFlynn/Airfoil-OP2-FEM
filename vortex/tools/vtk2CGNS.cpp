...
int main(int argc, char **argv) {
  ...
  // Write out CGNS file
  int file;
  if (cg_open("grid.cgns", CG_MODE_WRITE, &file)) {
    cg_error_exit();
  }
  ...
  for(int i = 0; i < numEdges; i++) {
    ...
    for(int j = i + 1; j < numEdges; j++) {
      if((p1 == edges[j * 6] && p2 == edges[j * 6 + 1]) || (p1 == edges[j * 6 + 1] && p2 == edges[j * 6]))
        cout << "Repeat edge on points" << endl;
      if((c1 == edges[j * 6 + 2] && c2 == edges[j * 6 + 3]) || (c1 == edges[j * 6 + 3] && c2 == edges[j * 6 + 2]))
        cout << "Repeat edge on cells" << endl;
    }
  }
}
